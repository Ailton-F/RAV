...
class Visita(db.Model):
    ...
    def __init__(self, id_voluntario, id_lar, nome_lar, dt_hr, motivo, status, qnt_pessoas):
    ...
    def __repr__(self):
        return f"visita('{self.dt_hr}, {self.motivo}, {self.status}, {self.qnt_pessoas}')"

class Notificacao(db.Model):
    __tablename__ = 'notificacao'
    id = db.Column(db.Integer, primary_key=True)
    id_remetente = db.Column(db.Integer, db.ForeignKey("usuario.id"))
    id_destino = db.Column(db.Integer, db.ForeignKey("usuario.id"))
    mensagem = db.Column(db.String(100))
    dt_hr = db.Column(db.DateTime, default=datetime.datetime.now())
    eacao = db.Column(db.Boolean)
    rs_notificacao_usuario = db.relationship(Usuario, foreign_keys=id_remetente)
    rs_notificacao_usuario2 = db.relationship(Usuario, foreign_keys=id_destino)

    def __init__(self, id_remetente, id_destino, mensagem, eacao):
